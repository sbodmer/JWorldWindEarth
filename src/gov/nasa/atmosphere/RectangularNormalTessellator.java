 ...
 *          <p/>
 *          Modified by Michael de Hoog to add simple normals based on globe ellipse, (globe.computeSurfaceNormalAtPoint()), also added more exact normal calculator for terrain tiles, see getNormals()
 */
public class RectangularNormalTessellator extends WWObjectImpl implements Tessellator {
	protected static class RenderInfo {
		...
		protected RenderInfo(int density, DoubleBuffer vertices, DoubleBuffer texCoords, DoubleBuffer normals, Vec4 refCenter) {
	...
	public static class RectTile implements SectorGeometry {
		...
		public RectTile(RectangularNormalTessellator tessellator, Extent extent, int level, int density, Sector sector, double cellSize) {
	...
	protected static final int DEFAULT_FREQUENCY = 2000;
	...
	protected static final Map<Integer, DoubleBuffer> parameterizations = new HashMap<Integer, DoubleBuffer>();
	protected static final Map<Integer, IntBuffer> indexLists = new HashMap<Integer, IntBuffer>();

	protected List<RectTile> topLevels;
	...
	protected long updateFrequency = DEFAULT_FREQUENCY;
	...
	protected List<RectTile> createTopLevelTiles(DrawContext dc) {
		List<RectTile> tops = new ArrayList<RectTile>(DEFAULT_NUM_LAT_SUBDIVISIONS * DEFAULT_NUM_LON_SUBDIVISIONS);
	...
	public RenderInfo buildVerts(DrawContext dc, RectTile tile, boolean makeSkirts) {
		...
		List<LatLon> latlons = this.computeLocations(tile);
		...
		IntBuffer indices = getIndices(density);
		DoubleBuffer norms = getNormals(density, verts, indices, refCenter);
	...
	protected List<LatLon> computeLocations(RectTile tile) {
		int density = tile.density;
		...
		List<LatLon> latlons = new ArrayList<LatLon>(numVertices);
	...
	protected long render(DrawContext dc, RectTile tile, int numTextureUnits) {
		...
		gl.glDrawElements(GL2.GL_TRIANGLE_STRIP, tile.ri.indices.limit(), GL2.GL_UNSIGNED_INT, tile.ri.indices.rewind());
	...
	protected void renderWireframe(DrawContext dc, RectTile tile, boolean showTriangles, boolean showTileBoundary) {
		...
		IntBuffer indices = getIndices(tile.ri.density);
		indices.rewind();
		...
		gl.glPushAttrib(GL2.GL_DEPTH_BUFFER_BIT | GL2.GL_POLYGON_BIT | GL2.GL_ENABLE_BIT | GL2.GL_CURRENT_BIT);
	...
	protected void renderTrianglesWithUniqueColors(DrawContext dc, RectTile tile) {
		...
		if (tile.ri.referenceCenter != null)
			dc.getView().pushReferenceCenter(dc, tile.ri.referenceCenter);
		...
		for (int i = 0; i < trianglesNum; i++) {
			...
			gl.glVertex3d(tile.ri.vertices.get(vIndex), tile.ri.vertices.get(vIndex + 1), tile.ri.vertices.get(vIndex + 2));
	...
	protected PickedObject resolvePick(DrawContext dc, RectTile tile, Point pickPoint) {
		...
		Vec4 v0 = new Vec4((tile.ri.vertices.get(vIndex++) + centerX), (tile.ri.vertices.get(vIndex++) + centerY), (tile.ri.vertices.get(vIndex) + centerZ));

		vIndex = 3 * tile.ri.indices.get(triangleIndex + 1);
		Vec4 v1 = new Vec4((tile.ri.vertices.get(vIndex++) + centerX), (tile.ri.vertices.get(vIndex++) + centerY), (tile.ri.vertices.get(vIndex) + centerZ));

		vIndex = 3 * tile.ri.indices.get(triangleIndex + 2);
		Vec4 v2 = new Vec4((tile.ri.vertices.get(vIndex++) + centerX), (tile.ri.vertices.get(vIndex++) + centerY), (tile.ri.vertices.get(vIndex) + centerZ));
	 ...
	 *
	 * @param tile
	 *            the <Code>RectTile</code> which geometry is to be tested for intersection.
	 * @param line
	 *            the ray for which an intersection is to be found.
	 *
	 * @return an array of <code>Intersection</code> sorted by increasing distance from the line origin, or null if no intersection was found.
	 */
	protected Intersection[] intersect(RectTile tile, Line line) {
		...
		for (int i = startIndice; i < endIndice; i += 2) {
			...
			Vec4 v1 = new Vec4(coords[vIndex++] + centerX, coords[vIndex++] + centerY, coords[vIndex] + centerZ);

			vIndex = 3 * indices[i + 2];
			Vec4 v2 = new Vec4(coords[vIndex++] + centerX, coords[vIndex++] + centerY, coords[vIndex] + centerZ);
			...
			Vec4 p;
			vIndex = 3 * indices[i];
			Vec4 v0 = new Vec4(coords[vIndex++] + centerX, coords[vIndex++] + centerY, coords[vIndex] + centerZ);

			vIndex = 3 * indices[i + 3];
			Vec4 v3 = new Vec4(coords[vIndex++] + centerX, coords[vIndex++] + centerY, coords[vIndex] + centerZ);
	...
	/**
	 * Determines if and where a <code>RectTile</code> geometry intersects the globe ellipsoid at a given elevation. The returned array of <code>Intersection</code> describes a list of individual
	 * segments - two <code>Intersection</code> for each, corresponding to each geometry triangle that intersects the given elevation.
	 *
	 * @param tile
	 *            the <Code>RectTile</code> which geometry is to be tested for intersection.
	 * @param elevation
	...
	protected Intersection[] intersect(RectTile tile, double elevation) {
		...
		if (tile.getExtent() instanceof Cylinder) {
			...
			if (!(globe.isPointAboveElevation(cylinder.getBottomCenter(), elevation) ^ globe.isPointAboveElevation(cylinder.getTopCenter(), elevation)))
				return null;
		}
	...
	/**
	 * Computes from a column (or row) number, and a given offset ranged [0,1] corresponding to the distance along the edge of this sector, where between this column and the next column the
	 * corresponding position will fall, in the range [0,1].
	 *
	 * @param start
	 *            the number of the column or row to the left, below or on this position
	 * @param decimal
	 *            the distance from the left or bottom of the current sector that this position falls
	 * @param density
	 *            the number of intervals along the sector's side
	 *
	 * @return a decimal ranged [0,1] representing the position between two columns or rows, rather than between two edges of the sector
	 */
	 ...
	 * 1</code> and at <code>yDec</code> offset from <code>row</code> to <code>row + 1</code>. Accounts for the diagonals.
	 *
	 * @param row
	 *            represents the row which corresponds to a <code>yDec</code> value of 0
	 * @param column
	 *            represents the column which corresponds to an <code>xDec</code> value of 0
	 * @param xDec
	 *            constrained to [0,1]
	 * @param yDec
	 *            constrained to [0,1]
	 ...
	 *
	 * @return a <code>Point</code> geometrically within or on the boundary of the quadrilateral whose bottom left corner is indexed by ( <code>row</code>, <code>column</code>)
	 */
	protected static Vec4 interpolate(int row, int column, double xDec, double yDec, RenderInfo ri) {
		...
		Vec4 bL = new Vec4(ri.vertices.get(bottomLeft), ri.vertices.get(bottomLeft + 1), ri.vertices.get(bottomLeft + 2));
		Vec4 bR = new Vec4(ri.vertices.get(bottomLeft + 3), ri.vertices.get(bottomLeft + 4), ri.vertices.get(bottomLeft + 5));

		bottomLeft += numVertsTimesThree;

		Vec4 tL = new Vec4(ri.vertices.get(bottomLeft), ri.vertices.get(bottomLeft + 1), ri.vertices.get(bottomLeft + 2));
		Vec4 tR = new Vec4(ri.vertices.get(bottomLeft + 3), ri.vertices.get(bottomLeft + 4), ri.vertices.get(bottomLeft + 5));

		return interpolate(bL, bR, tR, tL, xDec, yDec);
	}

	/**
	 * Calculates the point at (xDec, yDec) in the two triangles defined by {bL, bR, tL} and {bR, tR, tL}. If thought of as a quadrilateral, the diagonal runs from tL to bR. Of course, this isn't a
	 * quad, it's two triangles.
	 *
	 * @param bL
	...
	protected static Vec4 interpolate(Vec4 bL, Vec4 bR, Vec4 tR, Vec4 tL, double xDec, double yDec) {
		...
		if (pos == 1) {
			...
			return new Vec4(tL.x * yDec + bR.x * xDec, tL.y * yDec + bR.y * xDec, tL.z * yDec + bR.z * xDec);
		} else if (pos > 1) {
	...
	protected DoubleBuffer makeGeographicTexCoords(SectorGeometry sg, SectorGeometry.GeographicTextureCoordinateComputer computer) {
		if (sg == null) {
	...
	protected static IntBuffer getIndices(int density) {
		...
		IntBuffer buffer = indexLists.get(density);
		if (buffer != null)
			return buffer;
	...
	protected static DoubleBuffer getNormalsQuick(int density, DoubleBuffer vertices, Vec4 referenceCenter) {
		...
		for (int j = 1; j < side - 1; j++) {
			for (int i = 1; i < side - 1; i++) {
				...
				Vec4 n1 = p0.subtract3(p1).normalize3().cross3(p0.subtract3(p3).normalize3());
				Vec4 n2 = p0.subtract3(p2).normalize3().cross3(p0.subtract3(p4).normalize3());
				Vec4 normal = n1.add3(n2).normalize3();

				normals.put(index0 * 3, normal.x).put(index0 * 3 + 1, normal.y).put(index0 * 3 + 2, normal.z);
			}
		}
	...
	protected static Vec4 getVec4(int index, DoubleBuffer vertices, Vec4 referenceCenter) {
		return new Vec4(vertices.get(index * 3) + referenceCenter.x, vertices.get(index * 3 + 1) + referenceCenter.y, vertices.get(index * 3 + 2) + referenceCenter.z);
	}

	protected static void copyNormalInBuffer(int srcIndex, int dstIndex, DoubleBuffer normals) {
	...
	protected static DoubleBuffer getNormals(int density, DoubleBuffer vertices, IntBuffer indices, Vec4 referenceCenter) {
		int side = density + 3;
		int numVertices = side * side;
		...
		for (int i = 0; i < numFaces; i++) {
			...
			Vec4 v0 = new Vec4(vertices.get(index0 * 3) + centerX, vertices.get(index0 * 3 + 1) + centerY, vertices.get(index0 * 3 + 2) + centerZ);

			Vec4 v1 = new Vec4(vertices.get(index1 * 3) + centerX, vertices.get(index1 * 3 + 1) + centerY, vertices.get(index1 * 3 + 2) + centerZ);

			Vec4 v2 = new Vec4(vertices.get(index2 * 3) + centerX, vertices.get(index2 * 3 + 1) + centerY, vertices.get(index2 * 3 + 2) + centerZ);
		...
		for (int i = 0; i < numVertices; i++) {
			...
			int index = i * 3;
			normals.put(index++, norms[i].x).put(index++, norms[i].y).put(index, norms[i].z);
		}
	...
	/*
	 * protected static DoubleBuffer getNormals(int density, DoubleBuffer vertices, IntBuffer indices, Vec4 referenceCenter) { int numVertices = (density + 3) * (density + 3); int sideSize = density +
	 * 2; int numFaces = indices.limit() - 2; double centerX = referenceCenter.x; double centerY = referenceCenter.y; double centerZ = referenceCenter.z; // Create normal buffer DoubleBuffer normals =
	 * Buffers.newDirectDoubleBuffer(numVertices * 3); // Create per vertex normal lists List<List<Vec4>> normalLists = new ArrayList<List<Vec4>>(numVertices); for (int i = 0; i < numVertices; i++)
	 * normalLists.add(i, new ArrayList<Vec4>()); // Go through all faces in the strip and store normals in lists for (int i = 0; i < numFaces; i++) { int vIndex = 3 * indices.get(i); Vec4 v0 = new
	 * Vec4((vertices.get(vIndex++) + centerX), (vertices .get(vIndex++) + centerY), (vertices.get(vIndex) + centerZ));
	 * 
	 * vIndex = 3 * indices.get(i + 1); Vec4 v1 = new Vec4((vertices.get(vIndex++) + centerX), (vertices .get(vIndex++) + centerY), (vertices.get(vIndex) + centerZ));
	 * 
	 * vIndex = 3 * indices.get(i + 2); Vec4 v2 = new Vec4((vertices.get(vIndex++) + centerX), (vertices .get(vIndex++) + centerY), (vertices.get(vIndex) + centerZ));
	 * 
	 * // get triangle edge vectors and plane normal Vec4 e1 = v1.subtract3(v0); Vec4 e2 = v2.subtract3(v0); Vec4 N = e1.cross3(e2).normalize3(); // if N is 0, the triangle is degenerate
	 * 
	 * // Store the face's normal for each of the vertices that make up the face. normalLists.get(indices.get(i)).add(N); normalLists.get(indices.get(i + 1)).add(N); normalLists.get(indices.get(i +
	 * 2)).add(N); //System.out.println("Normal: " + N); }
	 * 
	 * // Now loop through each vertex, and average out all the normals stored. int idx = 0; for (int i = 0; i < numVertices; i++) { Vec4 normal = Vec4.ZERO; // Sum for (int j = 0; j <
	 * normalLists.get(i).size(); ++j) normal = normal.add3(normalLists.get(i).get(j)); // Average normal = normal.multiply3(1.0f / normalLists.get(i).size()) .normalize3(); // Fill normal buffer
	 * normals.put(idx++, normal.x); normals.put(idx++, normal.y); normals.put(idx++, normal.z); //System.out.println("Normal: " + normal + " - " + normalLists[i].size());
