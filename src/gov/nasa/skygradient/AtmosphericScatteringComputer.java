...
public class AtmosphericScatteringComputer {
	...
	private void interpolate(float[] p, float x, float y) {
		...
		for (int i = 0; i < nChannels; i++) {
			p[i] = opticalDepthBuffer[pValueOffset] * (1 - fRatioX) * (1 - fRatioY) + opticalDepthBuffer[pValueOffset + nChannels] * (fRatioX) * (1 - fRatioY)
					+ opticalDepthBuffer[pValueOffset + nChannels * nBufferWidth] * (1 - fRatioX) * (fRatioY) + opticalDepthBuffer[pValueOffset + nChannels * (nBufferWidth + 1)] * (fRatioX)
					* (fRatioY);
			pValueOffset++;
		}
	}
	...
	private void computeOpticalDepthBuffer() {
		...
		for (int nAngle = 0; nAngle < nSize; nAngle++) {
			...
			for (int nHeight = 0; nHeight < nSize; nHeight++) {
				...
				boolean bVisible = (fDet < 0 || (0.5f * (-B - (float) Math.sqrt(fDet)) <= 0) && (0.5f * (-B + (float) Math.sqrt(fDet)) <= 0));
